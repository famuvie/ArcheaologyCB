...
```{r data-summary, cache = FALSE, fig.cap = 'Histograms of measured variables.', fig.width = 5, fig.height = 7}
str(dataset)
summary(dataset)
...
```{r raw-data, cache = FALSE, fig.cap = 'Measurement locations and observed values', fig.width = 8, fig.height = 10}
...
```{r Ca-autokriging, include=FALSE}
## Auto-kriging
## Just a small test
library(automap)
dat.sp <- dataset
coordinates(dat.sp) <- ~x + y
autovg <- autofitVariogram(Ca ~ 1, dat.sp)

# automap::autofitVariogram uses gstat::variogram
# and fits models by minimizing the residual sum of squares
# wrt the empirical variogram

# compare the likelihood of these fits with the REML fit
# of the exponential model

vgmdl.test <- likfit(geodata = obs.gd,
                       data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.3,  # If 0.5 then cov.model changes to exponential
                       ini = c(10, 5),  # sigma^2 (partial sill) and phi (range parameter)
                       cov.model = "matern",
                       lik.method = "REML")

# The likelihood is indeed higher, but very similar.
# The problem is that the matern model does not converge
# with the cost-based distances.
...
# qplot(res[,1], res[,2]) + geom_abline(intercept=0, slope=1)
...
```{r foodremains-fix-variable}

## Compute specific geodata object for this variable
## in ordder to exclude the observations with NA which 
## cause problems both in variog and krige.conv 
## (although not in likfit)
obs.gd <- as.geodata(dataset[, -(1:3)], 
                     data.col = "foodremains")
na.idx <- is.na(dataset$foodremains)
which(na.idx)
```
...
```{r foodremains-euclidean-kriging, include = FALSE}

## compute euclidean (only) variogram
vg.std.foodremains <- variog(obs.gd)

## fitting variogram models
vgmdl.std.foodremains <- likfit(geodata = obs.gd,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       # kappa = 0.51,  # If 0.5 then cov.model changes to exponential
                       ini = c(10, 5),  # sigma^2 (partial sill) and phi (range parameter)
...
## Fitted parameters
par.tab <- data.frame(
  Euclidean = with(vgmdl.std.foodremains,
                   c(beta,
                     nugget,
                     sigmasq,
                     # kappa,
...
# Conventional Kriging, Euclidean distances
KC.std.foodremains = krige.control(obj.model = vgmdl.std.foodremains)
kriging.std.foodremains <- krige.conv(obs.gd,
                             locations = loc,
                             krige = KC.std.foodremains)

```

```{r foodremains-euclidean-variogram-plot, fig.width = 6,fig.height = 3, fig.cap = 'Empirical variogram and fitted model.'}

ggplot(vg.std.foodremains) + 
  geom_variogram(vgmdl.std.foodremains) + 
  ylab('semivariance foodremains')

```


```{r foodremains-euclidean-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Euclidean kriging prediction'}

p.foodremains <- ggplot(data.frame(loc, Prediction = kriging.std.foodremains$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r foodremains-cost-based-kriging, include=FALSE}


## compute cost-based empirical variogram
vg.cst.foodremains <- variog(obs.gd, dists.mat = ddm$obs[!na.idx, !na.idx])

## fitting variogram models
vgmdl.cst.foodremains <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.51,  # If 0.5 then cov.model changes to exponential
                       ...
                       cov.model = "exponential",
                       lik.method = "REML",
                       dists.mat = ddm$obs[!na.idx, !na.idx])

## Fitted parameters
par.tab <- cbind(par.tab,
                 data.frame(
                   Cost_based = with(vgmdl.cst.foodremains,
                                     c(beta,
                                       nugget,
                                       sigmasq,
                                       # kappa,
...
# Conventional Kriging, Euclidean distances
KC.cst.foodremains = krige.control(obj.model = vgmdl.cst.foodremains)
kriging.cst.foodremains <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.cst.foodremains,
                             dd.dists.mat = ddm$obs[!na.idx, !na.idx],
                             dl.dists.mat = ddm$loc[, !na.idx])

```



```{r foodremains-cost-based-variogram-plot, fig.width = 6, fig.height = 3, fig.cap = 'Empirical cost-based variogram and fitted model.'}

ggplot(vg.cst.foodremains) +
  geom_variogram(vgmdl.cst.foodremains) +
  ylab('semivariance foodremains')

```


```{r foodremains-cost-based-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Cost-based kriging prediction'}

p.foodremains <- ggplot(data.frame(loc, Prediction = kriging.cst.foodremains$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r foodremains-parameter-table}
kable(par.tab, digits = 2)
```


```{r foodremains-compare-variogram-plot, fig.width = 6, fig.height = 4, fig.cap = 'Empirical variogram and fitted models by method for food remains.'}

vg.both <- rbind(data.frame(vg.std.foodremains[1:3],
                            variable = 'foodremains',
                            method = 'Euclidean'),
                 data.frame(vg.cst.foodremains[1:3],
                            variable = 'foodremains',
                            method = 'Cost-based'))

names(vg.both) <- c('distance', 'semivariance', 'n', 'variable', 'method')

p.foodremains <- ggplot(filter(vg.both, variable == 'foodremains'),
               aes(distance, semivariance)) +
  geom_point(aes(size = n)) + 
  expand_limits(y=0) +
  geom_variogram(vgmdl.std.foodremains, variable = 'foodremains', method = 'Euclidean') +
  geom_variogram(vgmdl.cst.foodremains, variable = 'foodremains', method = 'Cost-based') +
  facet_grid(method ~ .) +
  theme_bw()
...
```{r foodremains-compare-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Comparison of Kriging estimates.'}

res.df.foodremains <- data.frame(
  loc, 
  method = c(rep('Classical', nrow(loc)),
             rep('Cost-based', nrow(loc)),
             rep('Difference', nrow(loc)),
             rep('Relative-difference', nrow(loc))),
  Prediction = c(kriging.std.foodremains$predict,
                 kriging.cst.foodremains$predict,
                 kriging.std.foodremains$predict-kriging.cst.foodremains$predict,
                 100*(kriging.std.foodremains$predict-kriging.cst.foodremains$predict)/kriging.cst.foodremains$predict)
)

p.foodremains <- 
  ggplot(filter(res.df.foodremains, method %in% c("Classical", "Cost-based")),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .1, color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
  scale_color_viridis() +
  facet_wrap(~method, ncol = 1) 
...
```{r foodremains-cost-based-vs-euclidean-prediction, fig.cap = 'Pointwise comparison of predictions by method.'}

p.foodremains <- data.frame(Euclidean  = kriging.std.foodremains$predict,
                   Cost_based = kriging.cst.foodremains$predict) %>% 
  sample_n(5e3) %>% 
  ggplot(aes(Euclidean, Cost_based)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = 'darkgray') +
...
```{r foodremains-map-kriging-difference, fig.width=10, fig.height=6, fig.cap='Relative Difference between the Euclidean and the cost-based predictions. Expressed as a percentage wrt Cost-based prediction.'}

pred.dif.foodremains <- 
  ggplot(filter(res.df.foodremains, method == 'Relative-difference'),
         aes(x, y)) + 
  geom_tile(aes(fill = Prediction, color = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = 2, color = 'lightgray') +
  geom_india() +
...
```{r foodremains-compare-kriging-errors, fig.width=10, fig.height=7, fig.cap = 'Comparison of prediction error by method.'}

res.var.foodremains <- data.frame(
  loc, 
  method = c(
    rep('Classical', nrow(loc)),
    rep('Cost-based', nrow(loc)),
    rep('Difference', nrow(loc)),
    rep('Relative-difference', nrow(loc))),
  Prediction = c(
    kriging.std.foodremains$krige.var,
    kriging.cst.foodremains$krige.var,
    kriging.std.foodremains$krige.var-kriging.cst.foodremains$krige.var,
    100*(kriging.std.foodremains$krige.var-kriging.cst.foodremains$krige.var)/kriging.cst.foodremains$krige.var)
)

ggplot(filter(res.var.foodremains, method %in% c("Classical", "Cost-based")),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r foodremains-map-kriging-difference-errors, fig.width=10, fig.height=6, fig.cap='Relative difference between the Euclidean and the cost-based prediction errors. Expressed as a percentage wrt Cost-based variance.'}

ggplot(filter(res.var.foodremains, method == 'Relative-difference'),
         aes(x, y)) + 
  geom_tile(aes(fill = Prediction, color = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = 5, color = 'lightgray') +
  geom_india() +
...
In the main area, the prediction errors are practically the same with both approaches.
In most of the area, the Cost-based approach is slightly more accurate (median 1%) achieving up to 1.4% of improvement in accuracy in the rightmost corners.

The drop of about -5% prediction variance for the Euclidean prediction in the other corners and the veranda area are a consequence of incorrectly assuming that observations at the other side of the wall are *close*.

Behind the walls, the Euclidean prediction error is unrealistically low (up to -23%) and is not represented.


## Leave-one-out Cross Validation (LOOCV)

```{r foodremains-loocv, fig.width = 4, fig.height = 3, fig.cap = 'Pointwise leave-one-out prediction error by method.'}
loocvdat <- obs.gd
# loocvdat$data <- variable
ddm.na <- list(obs = ddm$obs[!na.idx, !na.idx],
               loc = ddm$loc[, !na.idx])

res <- sapply(c('std', 'cst'), krige.loocv, loocvdat, ddm.na)
sink()   # Why I need this?
...
obs.gd <- as.geodata(dataset[, -(1:3)], 
                     data.col = "livingroom")
na.idx <- is.na(dataset$livingroom)
```
...
```{r livingroom-euclidean-kriging, include = FALSE}

## compute euclidean (only) variogram
vg.std.livingroom <- variog(obs.gd)

## fitting variogram models
vgmdl.std.livingroom <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       # kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
## Fitted parameters
par.tab <- data.frame(
  Euclidean = with(vgmdl.std.livingroom,
                   c(beta,
                     nugget,
                     sigmasq,
                     # kappa,
...
# Conventional Kriging, Euclidean distances
KC.std.livingroom = krige.control(obj.model = vgmdl.std.livingroom)
kriging.std.livingroom <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.std.livingroom)

```

```{r livingroom-euclidean-variogram-plot, fig.width = 6,fig.height = 3, fig.cap = 'Empirical variogram and fitted model.'}

ggplot(vg.std.livingroom) + 
  geom_variogram(vgmdl.std.livingroom) + 
  ylab('semivariance livingroom')

```


```{r livingroom-euclidean-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Euclidean kriging prediction'}

p.livingroom <- ggplot(data.frame(loc, Prediction = kriging.std.livingroom$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r livingroom-cost-based-kriging, include=FALSE}


## compute cost-based empirical variogram
vg.cst.livingroom <- variog(obs.gd, dists.mat = ddm$obs[!na.idx, !na.idx])

## fitting variogram models
vgmdl.cst.livingroom <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
par.tab <- cbind(par.tab,
                 data.frame(
                   Cost_based = with(vgmdl.cst.livingroom,
                                     c(beta,
                                       nugget,
                                       sigmasq,
                                       # kappa,
...
# Conventional Kriging, Euclidean distances
KC.cst.livingroom = krige.control(obj.model = vgmdl.cst.livingroom)
kriging.cst.livingroom <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.cst.livingroom,
                             dd.dists.mat = ddm$obs[!na.idx, !na.idx],
                             dl.dists.mat = ddm$loc[, !na.idx])
...
```{r livingroom-cost-based-variogram-plot, fig.width = 6, fig.height = 3, fig.cap = 'Empirical cost-based variogram and fitted model.'}

ggplot(vg.cst.livingroom) +
  geom_variogram(vgmdl.cst.livingroom) +
  ylab('semivariance livingroom')

```


```{r livingroom-cost-based-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Cost-based kriging prediction'}

p.livingroom <- ggplot(data.frame(loc, Prediction = kriging.cst.livingroom$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r livingroom-parameter-table}
kable(par.tab, digits = 2)
```


```{r livingroom-compare-variogram-plot, fig.width = 6, fig.height = 4, fig.cap = 'Empirical variogram and fitted models by method for living room.'}

vg.both <- rbind(data.frame(vg.std.livingroom[1:3],
                            variable = 'livingroom',
                            method = 'Euclidean'),
                 data.frame(vg.cst.livingroom[1:3],
                            variable = 'livingroom',
                            method = 'Cost-based'))

names(vg.both) <- c('distance', 'semivariance', 'n', 'variable', 'method')

p.livingroom <- ggplot(filter(vg.both, variable == 'livingroom'),
               aes(distance, semivariance)) +
  geom_point(aes(size = n)) + 
  expand_limits(y=0) +
  geom_variogram(vgmdl.std.livingroom, variable = 'livingroom', method = 'Euclidean') +
  geom_variogram(vgmdl.cst.livingroom, variable = 'livingroom', method = 'Cost-based') +
  facet_grid(method ~ .) +
  theme_bw()
...
```{r livingroom-compare-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Comparison of Kriging estimates.'}

res.df.livingroom <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.livingroom$predict,
                                          kriging.cst.livingroom$predict,
                                          kriging.std.livingroom$predict-kriging.cst.livingroom$predict))

p.livingroom <- 
  ggplot(filter(res.df.livingroom, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r livingroom-cost-based-vs-euclidean-prediction, fig.cap = 'Pointwise comparison of predictions by method.'}

p.livingroom <- data.frame(Euclidean  = kriging.std.livingroom$predict,
                   Cost_based = kriging.cst.livingroom$predict) %>% 
  sample_n(5e3) %>% 
  ggplot(aes(Euclidean, Cost_based)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = 'darkgray') +
...
```{r livingroom-map-kriging-difference, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based predictions.'}
...
```{r livingroom-compare-kriging-errors, fig.width=10, fig.height=7, fig.cap = 'Comparison of prediction error by method.'}

res.var.livingroom <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.livingroom$krige.var,
                                          kriging.cst.livingroom$krige.var,
                                          kriging.std.livingroom$krige.var-kriging.cst.livingroom$krige.var))

ggplot(filter(res.var.livingroom, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r livingroom-map-kriging-difference-errors, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based prediction errors'}
...
```{r livingroom-loocv, fig.width = 4, fig.height = 3, fig.cap = 'Pointwise leave-one-out prediction error by method.'}
loocvdat <- obs.gd
# loocvdat$data <- variable
ddm.na <- list(obs = ddm$obs[!na.idx, !na.idx],
               loc = ddm$loc[, !na.idx])
...
# Analysis of enclosed spaces

All these results look very much like Calcium results, at a different scale.


```{r enclosedspaces-fix-variable}
...
## (although not in likfit)
obs.gd <- as.geodata(dataset[, -(1:3)], 
                     data.col = "enclosedspaces")
na.idx <- is.na(dataset$enclosedspaces)
```
...
```{r enclosedspaces-euclidean-kriging, include = FALSE}

## compute euclidean (only) variogram
vg.std.enclosedspaces <- variog(obs.gd)

## fitting variogram models
vgmdl.std.enclosedspaces <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       # kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
## Fitted parameters
par.tab <- data.frame(
  Euclidean = with(vgmdl.std.enclosedspaces,
                   c(beta,
                     nugget,
                     sigmasq,
                     # kappa,
...
# Conventional Kriging, Euclidean distances
KC.std.enclosedspaces = krige.control(obj.model = vgmdl.std.enclosedspaces)
kriging.std.enclosedspaces <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.std.enclosedspaces)

```

```{r enclosedspaces-euclidean-variogram-plot, fig.width = 6,fig.height = 3, fig.cap = 'Empirical variogram and fitted model.'}

ggplot(vg.std.enclosedspaces) + 
  geom_variogram(vgmdl.std.enclosedspaces) + 
  ylab('semivariance enclosedspaces')

```


```{r enclosedspaces-euclidean-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Euclidean kriging prediction'}

p.enclosedspaces <- ggplot(data.frame(loc, Prediction = kriging.std.enclosedspaces$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r enclosedspaces-cost-based-kriging, include=FALSE}


## compute cost-based empirical variogram
vg.cst.enclosedspaces <- variog(obs.gd, dists.mat = ddm$obs[!na.idx, !na.idx])

## fitting variogram models
vgmdl.cst.enclosedspaces <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
par.tab <- cbind(par.tab,
                 data.frame(
                   Cost_based = with(vgmdl.cst.enclosedspaces,
                                     c(beta,
                                       nugget,
                                       sigmasq,
                                       # kappa,
...
# Conventional Kriging, Euclidean distances
KC.cst.enclosedspaces = krige.control(obj.model = vgmdl.cst.enclosedspaces)
kriging.cst.enclosedspaces <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.cst.enclosedspaces,
                             dd.dists.mat = ddm$obs[!na.idx, !na.idx],
                             dl.dists.mat = ddm$loc[, !na.idx])
...
```{r enclosedspaces-cost-based-variogram-plot, fig.width = 6, fig.height = 3, fig.cap = 'Empirical cost-based variogram and fitted model.'}

ggplot(vg.cst.enclosedspaces) +
  geom_variogram(vgmdl.cst.enclosedspaces) +
  ylab('semivariance enclosedspaces')

```


```{r enclosedspaces-cost-based-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Cost-based kriging prediction'}

p.enclosedspaces <- ggplot(data.frame(loc, Prediction = kriging.cst.enclosedspaces$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r enclosedspaces-parameter-table}
kable(par.tab, digits = 2)
```


```{r enclosedspaces-compare-variogram-plot, fig.width = 6, fig.height = 4, fig.cap = 'Empirical variogram and fitted models by method for enclosed spaces.'}

vg.both <- rbind(data.frame(vg.std.enclosedspaces[1:3],
                            variable = 'enclosedspaces',
                            method = 'Euclidean'),
                 data.frame(vg.cst.enclosedspaces[1:3],
                            variable = 'enclosedspaces',
                            method = 'Cost-based'))

names(vg.both) <- c('distance', 'semivariance', 'n', 'variable', 'method')

p.enclosedspaces <- ggplot(filter(vg.both, variable == 'enclosedspaces'),
               aes(distance, semivariance)) +
  geom_point(aes(size = n)) + 
  expand_limits(y=0) +
  geom_variogram(vgmdl.std.enclosedspaces, variable = 'enclosedspaces', method = 'Euclidean') +
  geom_variogram(vgmdl.cst.enclosedspaces, variable = 'enclosedspaces', method = 'Cost-based') +
  facet_grid(method ~ .) +
  theme_bw()
...
```{r enclosedspaces-compare-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Comparison of Kriging estimates.'}

res.df.enclosedspaces <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.enclosedspaces$predict,
                                          kriging.cst.enclosedspaces$predict,
                                          kriging.std.enclosedspaces$predict-kriging.cst.enclosedspaces$predict))

p.enclosedspaces <- 
  ggplot(filter(res.df.enclosedspaces, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r enclosedspaces-cost-based-vs-euclidean-prediction, fig.cap = 'Pointwise comparison of predictions by method.'}

p.enclosedspaces <- data.frame(Euclidean  = kriging.std.enclosedspaces$predict,
                   Cost_based = kriging.cst.enclosedspaces$predict) %>% 
  sample_n(5e3) %>% 
  ggplot(aes(Euclidean, Cost_based)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = 'darkgray') +
...
```{r enclosedspaces-map-kriging-difference, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based predictions.'}

pred.dif.enclosedspaces <- 
  ggplot(filter(res.df.enclosedspaces, method == 'Difference'),
         aes(x, y)) + 
  geom_tile(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
...
```{r enclosedspaces-compare-kriging-errors, fig.width=10, fig.height=7, fig.cap = 'Comparison of prediction error by method.'}

res.var.enclosedspaces <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.enclosedspaces$krige.var,
                                          kriging.cst.enclosedspaces$krige.var,
                                          kriging.std.enclosedspaces$krige.var-kriging.cst.enclosedspaces$krige.var))

ggplot(filter(res.var.enclosedspaces, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r enclosedspaces-map-kriging-difference-errors, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based prediction errors'}
...
```{r enclosedspaces-loocv, fig.width = 4, fig.height = 3, fig.cap = 'Pointwise leave-one-out prediction error by method.'}
loocvdat <- obs.gd
# loocvdat$data <- variable
ddm.na <- list(obs = ddm$obs[!na.idx, !na.idx],
               loc = ddm$loc[, !na.idx])
...
# Analysis of burningareas

```{r burningareas-fix-variable}
...
## (although not in likfit)
obs.gd <- as.geodata(dataset[, -(1:3)], 
                     data.col = "burningareas")
na.idx <- is.na(dataset$burningareas)
```
...
```{r burningareas-euclidean-kriging, include = FALSE}

## compute euclidean (only) variogram
vg.std.burningareas <- variog(obs.gd)

## fitting variogram models
vgmdl.std.burningareas <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       # kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
## Fitted parameters
par.tab <- data.frame(
  Euclidean = with(vgmdl.std.burningareas,
                   c(beta,
                     nugget,
                     sigmasq,
                     # kappa,
...
# Conventional Kriging, Euclidean distances
KC.std.burningareas = krige.control(obj.model = vgmdl.std.burningareas)
kriging.std.burningareas <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.std.burningareas)

```

```{r burningareas-euclidean-variogram-plot, fig.width = 6,fig.height = 3, fig.cap = 'Empirical variogram and fitted model.'}

ggplot(vg.std.burningareas) + 
  geom_variogram(vgmdl.std.burningareas) + 
  ylab('semivariance burningareas')

```


```{r burningareas-euclidean-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Euclidean kriging prediction'}

p.burningareas <- ggplot(data.frame(loc, Prediction = kriging.std.burningareas$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r burningareas-cost-based-kriging, include=FALSE}


## compute cost-based empirical variogram
vg.cst.burningareas <- variog(obs.gd, dists.mat = ddm$obs[!na.idx, !na.idx])

## fitting variogram models
vgmdl.cst.burningareas <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
par.tab <- cbind(par.tab,
                 data.frame(
                   Cost_based = with(vgmdl.cst.burningareas,
                                     c(beta,
                                       nugget,
                                       sigmasq,
                                       # kappa,
...
# Conventional Kriging, Euclidean distances
KC.cst.burningareas = krige.control(obj.model = vgmdl.cst.burningareas)
kriging.cst.burningareas <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.cst.burningareas,
                             dd.dists.mat = ddm$obs[!na.idx, !na.idx],
                             dl.dists.mat = ddm$loc[, !na.idx])
...
```{r burningareas-cost-based-variogram-plot, fig.width = 6, fig.height = 3, fig.cap = 'Empirical cost-based variogram and fitted model.'}

ggplot(vg.cst.burningareas) +
  geom_variogram(vgmdl.cst.burningareas) +
  ylab('semivariance burningareas')

```


```{r burningareas-cost-based-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Cost-based kriging prediction'}

p.burningareas <- ggplot(data.frame(loc, Prediction = kriging.cst.burningareas$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r burningareas-parameter-table}
kable(par.tab, digits = 2)
```


```{r burningareas-compare-variogram-plot, fig.width = 6, fig.height = 4, fig.cap = 'Empirical variogram and fitted models by method for burningareas.'}

vg.both <- rbind(data.frame(vg.std.burningareas[1:3],
                            variable = 'burningareas',
                            method = 'Euclidean'),
                 data.frame(vg.cst.burningareas[1:3],
                            variable = 'burningareas',
                            method = 'Cost-based'))

names(vg.both) <- c('distance', 'semivariance', 'n', 'variable', 'method')

p.burningareas <- ggplot(filter(vg.both, variable == 'burningareas'),
               aes(distance, semivariance)) +
  geom_point(aes(size = n)) + 
  expand_limits(y=0) +
  geom_variogram(vgmdl.std.burningareas, variable = 'burningareas', method = 'Euclidean') +
  geom_variogram(vgmdl.cst.burningareas, variable = 'burningareas', method = 'Cost-based') +
  facet_grid(method ~ .) +
  theme_bw()
...
```{r burningareas-compare-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Comparison of Kriging estimates.'}

res.df.burningareas <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.burningareas$predict,
                                          kriging.cst.burningareas$predict,
                                          kriging.std.burningareas$predict-kriging.cst.burningareas$predict))

p.burningareas <- 
  ggplot(filter(res.df.burningareas, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r burningareas-cost-based-vs-euclidean-prediction, fig.cap = 'Pointwise comparison of predictions by method.'}

p.burningareas <- data.frame(Euclidean  = kriging.std.burningareas$predict,
                   Cost_based = kriging.cst.burningareas$predict) %>% 
  sample_n(5e3) %>% 
  ggplot(aes(Euclidean, Cost_based)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = 'darkgray') +
...
```{r burningareas-map-kriging-difference, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based predictions.'}

pred.dif.burningareas <- 
  ggplot(filter(res.df.burningareas, method == 'Difference'),
         aes(x, y)) + 
  geom_tile(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
...
```{r burningareas-compare-kriging-errors, fig.width=10, fig.height=7, fig.cap = 'Comparison of prediction error by method.'}

res.var.burningareas <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.burningareas$krige.var,
                                          kriging.cst.burningareas$krige.var,
                                          kriging.std.burningareas$krige.var-kriging.cst.burningareas$krige.var))

ggplot(filter(res.var.burningareas, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r burningareas-map-kriging-difference-errors, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based prediction errors'}
...
```{r burningareas-loocv, fig.width = 4, fig.height = 3, fig.cap = 'Pointwise leave-one-out prediction error by method.'}
loocvdat <- obs.gd
# loocvdat$data <- variable
ddm.na <- list(obs = ddm$obs[!na.idx, !na.idx],
               loc = ddm$loc[, !na.idx])
...
```{r ashes-euclidean-kriging, include = FALSE}

## compute euclidean (only) variogram
vg.std.ashes <- variog(obs.gd)

## fitting variogram models
vgmdl.std.ashes <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       # kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
## Fitted parameters
par.tab <- data.frame(
  Euclidean = with(vgmdl.std.ashes,
                   c(beta,
                     nugget,
                     sigmasq,
                     # kappa,
...
# Conventional Kriging, Euclidean distances
KC.std.ashes = krige.control(obj.model = vgmdl.std.ashes)
kriging.std.ashes <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.std.ashes)
...
```{r ashes-euclidean-variogram-plot, fig.width = 6,fig.height = 3, fig.cap = 'Empirical variogram and fitted model.'}

ggplot(vg.std.ashes) + 
  geom_variogram(vgmdl.std.ashes) + 
  ylab('semivariance ashes')

```


```{r ashes-euclidean-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Euclidean kriging prediction'}

p.ashes <- ggplot(data.frame(loc, Prediction = kriging.std.ashes$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r ashes-cost-based-kriging, include=FALSE}


## compute cost-based empirical variogram
vg.cst.ashes <- variog(obs.gd, dists.mat = ddm$obs[!na.idx, !na.idx])

## fitting variogram models
vgmdl.cst.ashes <- likfit(geodata = obs.gd,
                       # data = variable,
                       fix.nugget = FALSE,
                       fix.kappa = FALSE,
                       kappa = 0.51,  # If 0.5 then cov.model changes to exponential
...
par.tab <- cbind(par.tab,
                 data.frame(
                   Cost_based = with(vgmdl.cst.ashes,
                                     c(beta,
                                       nugget,
                                       sigmasq,
                                       # kappa,
...
# Conventional Kriging, Euclidean distances
KC.cst.ashes = krige.control(obj.model = vgmdl.cst.ashes)
kriging.cst.ashes <- krige.conv(obs.gd,
                             # data = variable,
                             locations = loc,
                             krige = KC.cst.ashes,
                             dd.dists.mat = ddm$obs[!na.idx, !na.idx],
...
```{r ashes-cost-based-variogram-plot, fig.width = 6, fig.height = 3, fig.cap = 'Empirical cost-based variogram and fitted model.'}

ggplot(vg.cst.ashes) +
  geom_variogram(vgmdl.cst.ashes) +
  ylab('semivariance ashes')

```


```{r ashes-cost-based-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Cost-based kriging prediction'}

p.ashes <- ggplot(data.frame(loc, Prediction = kriging.cst.ashes$predict), aes(x, y)) +
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), color = 'lightgray') +
  geom_india() +
  scale_fill_viridis() +
...
```{r ashes-parameter-table}
kable(par.tab, digits = 2)
```


```{r ashes-compare-variogram-plot, fig.width = 6, fig.height = 4, fig.cap = 'Empirical variogram and fitted models by method for ashes.'}

vg.both <- rbind(data.frame(vg.std.ashes[1:3],
                            variable = 'ashes',
                            method = 'Euclidean'),
                 data.frame(vg.cst.ashes[1:3],
                            variable = 'ashes',
                            method = 'Cost-based'))

names(vg.both) <- c('distance', 'semivariance', 'n', 'variable', 'method')

p.ashes <- ggplot(filter(vg.both, variable == 'ashes'),
               aes(distance, semivariance)) +
  geom_point(aes(size = n)) + 
  expand_limits(y=0) +
  geom_variogram(vgmdl.std.ashes, variable = 'ashes', method = 'Euclidean') +
  geom_variogram(vgmdl.cst.ashes, variable = 'ashes', method = 'Cost-based') +
  facet_grid(method ~ .) +
  theme_bw()
...
```{r ashes-compare-kriging-estimates, fig.width=10, fig.height=7, fig.cap = 'Comparison of Kriging estimates.'}

res.df.ashes <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.ashes$predict,
                                          kriging.cst.ashes$predict,
                                          kriging.std.ashes$predict-kriging.cst.ashes$predict))

p.ashes <- 
  ggplot(filter(res.df.ashes, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r ashes-cost-based-vs-euclidean-prediction, fig.cap = 'Pointwise comparison of predictions by method.'}

p.ashes <- data.frame(Euclidean  = kriging.std.ashes$predict,
                   Cost_based = kriging.cst.ashes$predict) %>% 
  sample_n(5e3) %>% 
  ggplot(aes(Euclidean, Cost_based)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = 'darkgray') +
...
```{r ashes-map-kriging-difference, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based predictions.'}
...
```{r ashes-compare-kriging-errors, fig.width=10, fig.height=7, fig.cap = 'Comparison of prediction error by method.'}

res.var.ashes <- data.frame(loc, 
                        method = c(rep('Classical', nrow(loc)),
                                   rep('Cost-based', nrow(loc)),
                                   rep('Difference', nrow(loc))),
                        Prediction = c(kriging.std.ashes$krige.var,
                                          kriging.cst.ashes$krige.var,
                                          kriging.std.ashes$krige.var-kriging.cst.ashes$krige.var))

ggplot(filter(res.var.ashes, method != 'Difference'),
         aes(x, y)) + 
  geom_raster(aes(fill = Prediction, colour = Prediction)) +
  stat_contour(aes(z = Prediction), binwidth = .5, color = 'lightgray') +
  geom_india() +
...
```{r ashes-map-kriging-difference-errors, fig.width=10, fig.height=6, fig.cap='Difference between the Euclidean and the cost-based prediction errors'}
...
```{r ashes-loocv, fig.width = 4, fig.height = 3, fig.cap = 'Pointwise leave-one-out prediction error by method.'}
loocvdat <- obs.gd
# loocvdat$data <- variable
ddm.na <- list(obs = ddm$obs[!na.idx, !na.idx],
               loc = ddm$loc[, !na.idx])
...
grid.arrange(
  ...
  ggplot(resid.Ca, aes(sample = x)) +
    geom_point(stat = 'qq') +
    geom_abline(aes(intercept = int, slope = slp), data = dat.lines) +
    facet_wrap(~ model)
)
